...
/**
 * xdbgvip_get_format_by_code - Retrieve format information for a media bus code
...
EXPORT_SYMBOL_GPL(xdbgvip_get_format_by_code);

/**
 * xdbgvip_get_format_by_fourcc - Retrieve format information for a 4CC
...
EXPORT_SYMBOL_GPL(xdbgvip_get_format_by_fourcc);

/**
 * xdbgvip_bpl_scaling_factor - Retrieve bpl scaling factor for a 4CC
 * @fourcc: the format 4CC
...
EXPORT_SYMBOL_GPL(xdbgvip_bpl_scaling_factor);

/**
 * xdbgvip_width_padding_factor - Retrieve width's padding factor for a 4CC
 * @fourcc: the format 4CC
...
EXPORT_SYMBOL_GPL(xdbgvip_width_padding_factor);

/**
 * xdbgvip_of_get_format - Parse a device tree node and return format information
...
EXPORT_SYMBOL_GPL(xdbgvip_of_get_format);

/**
 * xdbgvip_set_format_size - Set the media bus frame format size
...
EXPORT_SYMBOL_GPL(xdbgvip_set_format_size);

/**
 * xdbgvip_clr_or_set - Clear or set the register with a bitmask
...
void xdbgvip_clr_or_set(struct xvip_device *xvip, u32 addr, u32 mask, bool set)
{
	...
	reg = xdbgvip_read(xvip, addr);
	reg = set ? reg | mask : reg & ~mask;
	xdbgvip_write(xvip, addr, reg);
}
EXPORT_SYMBOL_GPL(xdbgvip_clr_or_set);

/**
 * xdbgvip_clr_and_set - Clear and set the register with a bitmask
 * @xvip: Xilinx Video IP device
 * @addr: address of register
...
EXPORT_SYMBOL_GPL(xdbgvip_clr_and_set);
...
/**
 * xdbgvip_enum_mbus_code - Enumerate the media format code
...
EXPORT_SYMBOL_GPL(xdbgvip_enum_mbus_code);

/**
 * xdbgvip_enum_frame_size - Enumerate the media bus frame size
 * @subdev: V4L2 subdevice
 * @sd_state: V4L2 subdev state
...
EXPORT_SYMBOL_GPL(xdbgvip_enum_frame_size);
