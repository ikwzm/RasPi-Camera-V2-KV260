...
{
	xdbgvip_write(&xcsc->xvip, reg, data);
}
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	struct v4l2_mbus_framefmt *format;
        ...
        dev_info(xcsc->xvip.dev, "%s start\n", __func__);
	format = __xcsc_get_pad_format(xcsc, sd_state, fmt->pad, fmt->which);
	if (!format) {
                dev_info(xcsc->xvip.dev, "%s done(-EINVAL)\n", __func__);
		return -EINVAL;
        }

	fmt->format = *format;
        dev_info(xcsc->xvip.dev, "%s done(0)\n", __func__);
	return 0;
}
...
static const struct v4l2_subdev_pad_ops xcsc_pad_ops = {
	.enum_mbus_code = xdbgvip_enum_mbus_code,
	.enum_frame_size = xdbgvip_enum_frame_size,
...
static int xcsc_parse_of(struct xcsc_dev *xcsc)
{
	...
	for_each_child_of_node(ports, port) {
		if (port->name && (of_node_cmp(port->name, "port") == 0)) {
			vip_format = xdbgvip_of_get_format(port);
...
static int xcsc_probe(struct platform_device *pdev)
{
	...
	gpiod_set_value_cansleep(xcsc->rst_gpio, XCSC_RESET_DEASSERT);
	rval = xdbgvip_init_resources(&xcsc->xvip);
	if (rval < 0)
		return rval;
...
media_error:
	xdbgvip_cleanup_resources(&xcsc->xvip);
	return rval;
}
...
static const struct of_device_id xdbg_csc_of_id_table[] = {
	{.compatible = "xlnx-dbg,v-vpss-csc"},
	{ }
};

MODULE_DEVICE_TABLE(of, xdbg_csc_of_id_table);

static struct platform_driver xdbg_csc_driver = {
	.driver = {
		.name = "xilinx-debug-vpss-csc",
		.of_match_table = xdbg_csc_of_id_table,
	},
	.probe = xcsc_probe,
	.remove = xcsc_remove,
};

module_platform_driver(xdbg_csc_driver);
