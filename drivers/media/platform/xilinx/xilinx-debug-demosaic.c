...
static inline u32 xdmsc_read(struct xdmsc_dev *xdmsc, u32 reg)
{
	...
	data = xdbgvip_read(&xdmsc->xvip, reg);
...
{
	xdbgvip_write(&xdmsc->xvip, reg, data);
	dev_dbg(xdmsc->xvip.dev,
...
static int xdmsc_probe(struct platform_device *pdev)
{
	...
	if (rval < 0)
		return rval;
	rval = xdbgvip_init_resources(&xdmsc->xvip);
	if (rval)
		return -EIO;
...
media_error:
	xdbgvip_cleanup_resources(&xdmsc->xvip);
	return rval;
}
...
static const struct of_device_id xdbg_dmsc_of_id_table[] = {
	{.compatible = "xlnx-dbg,v-demosaic"},
	{ }
};
MODULE_DEVICE_TABLE(of, xdbg_dmsc_of_id_table);

static struct platform_driver xdbg_dmsc_driver = {
	.driver = {
		.name = "xilinx-debug-demosaic",
		.of_match_table = xdbg_dmsc_of_id_table,
	},
	.probe = xdmsc_probe,
	.remove = xdmsc_remove,
...
module_platform_driver(xdbg_dmsc_driver);
