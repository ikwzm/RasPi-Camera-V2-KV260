...
#ifndef __XILINX_DEBUG_VIP_H__
#define __XILINX_DEBUG_VIP_H__
...
const struct xvip_video_format *xdbgvip_get_format_by_code(unsigned int code);
const struct xvip_video_format *xdbgvip_get_format_by_fourcc(u32 fourcc);
const struct xvip_video_format *xdbgvip_of_get_format(struct device_node *node);
void xdbgvip_bpl_scaling_factor(u32 fourcc, u32 *numerator, u32 *denominator);
void xdbgvip_width_padding_factor(u32 fourcc, u32 *numerator, u32 *denominator);
void xdbgvip_set_format_size(struct v4l2_mbus_framefmt *format,
			  const struct v4l2_subdev_format *fmt);
int xdbgvip_enum_mbus_code(struct v4l2_subdev *subdev,
			struct v4l2_subdev_state *sd_state,
			struct v4l2_subdev_mbus_code_enum *code);
int xdbgvip_enum_frame_size(struct v4l2_subdev *subdev,
			 struct v4l2_subdev_state *sd_state,
			 struct v4l2_subdev_frame_size_enum *fse);

static inline u32 xdbgvip_read(struct xvip_device *xvip, u32 addr)
{
	return ioread32(xvip->iomem + addr);
}

static inline void xdbgvip_write(struct xvip_device *xvip, u32 addr, u32 value)
{
	iowrite32(value, xvip->iomem + addr);
}

static inline void xdbgvip_clr(struct xvip_device *xvip, u32 addr, u32 clr)
{
	xdbgvip_write(xvip, addr, xdbgvip_read(xvip, addr) & ~clr);
}

static inline void xdbgvip_set(struct xvip_device *xvip, u32 addr, u32 set)
{
	xdbgvip_write(xvip, addr, xdbgvip_read(xvip, addr) | set);
}

void xdbgvip_clr_or_set(struct xvip_device *xvip, u32 addr, u32 mask, bool set);
void xdbgvip_clr_and_set(struct xvip_device *xvip, u32 addr, u32 clr, u32 set);

int xdbgvip_init_resources(struct xvip_device *xvip);
void xdbgvip_cleanup_resources(struct xvip_device *xvip);

static inline void xdbgvip_reset(struct xvip_device *xvip)
{
	xdbgvip_write(xvip, XVIP_CTRL_CONTROL, XVIP_CTRL_CONTROL_SW_RESET);
}

static inline void xdbgvip_start(struct xvip_device *xvip)
{
	xdbgvip_set(xvip, XVIP_CTRL_CONTROL,
		 XVIP_CTRL_CONTROL_SW_ENABLE | XVIP_CTRL_CONTROL_REG_UPDATE);
}

static inline void xdbgvip_stop(struct xvip_device *xvip)
{
	xdbgvip_clr(xvip, XVIP_CTRL_CONTROL, XVIP_CTRL_CONTROL_SW_ENABLE);
}

static inline void xdbgvip_resume(struct xvip_device *xvip)
{
	xdbgvip_write(xvip, XVIP_CTRL_CONTROL,
		   xvip->saved_ctrl | XVIP_CTRL_CONTROL_SW_ENABLE);
}

static inline void xdbgvip_suspend(struct xvip_device *xvip)
{
	xvip->saved_ctrl = xdbgvip_read(xvip, XVIP_CTRL_CONTROL);
	xdbgvip_write(xvip, XVIP_CTRL_CONTROL,
		   xvip->saved_ctrl & ~XVIP_CTRL_CONTROL_SW_ENABLE);
}

static inline void xdbgvip_set_frame_size(struct xvip_device *xvip,
				       const struct v4l2_mbus_framefmt *format)
{
	xdbgvip_write(xvip, XVIP_ACTIVE_SIZE,
		   (format->height << XVIP_ACTIVE_VSIZE_SHIFT) |
		   (format->width << XVIP_ACTIVE_HSIZE_SHIFT));
}

static inline void xdbgvip_get_frame_size(struct xvip_device *xvip,
				       struct v4l2_mbus_framefmt *format)
{
	u32 reg;

	reg = xdbgvip_read(xvip, XVIP_ACTIVE_SIZE);
...
static inline void xdbgvip_enable_reg_update(struct xvip_device *xvip)
{
	xdbgvip_set(xvip, XVIP_CTRL_CONTROL, XVIP_CTRL_CONTROL_REG_UPDATE);
}

static inline void xdbgvip_disable_reg_update(struct xvip_device *xvip)
{
	xdbgvip_clr(xvip, XVIP_CTRL_CONTROL, XVIP_CTRL_CONTROL_REG_UPDATE);
}

static inline void xdbgvip_print_version(struct xvip_device *xvip)
{
	u32 version;

	version = xdbgvip_read(xvip, XVIP_CTRL_VERSION);
