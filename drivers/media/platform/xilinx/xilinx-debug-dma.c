...
#include "xilinx-debug-dma.h"
#include "xilinx-debug-vip.h"
#include "xilinx-debug-vipp.h"
...
static int xvip_dma_verify_format(struct xvip_dma *dma)
{
	...
	int ret;
        ...
        dev_info(dma->xdev->dev, "%s start\n", __func__);
        
        dev_info(dma->xdev->dev, "%s xvip_dma_remote_subdev() start\n", __func__);
	subdev = xvip_dma_remote_subdev(&dma->pad, &fmt.pad);
	if (!subdev) {
                dev_info(dma->xdev->dev, "%s xvip_dma_remote_subdev() done(-EPIPE)\n", __func__);
		return -EPIPE;
        } else {
                dev_info(dma->xdev->dev, "%s xvip_dma_remote_subdev() done(Ok)\n", __func__);
        }          

	fmt.which = V4L2_SUBDEV_FORMAT_ACTIVE;
        dev_info(dma->xdev->dev, "%s v4l2_subdev_call() start\n", __func__);
	ret = v4l2_subdev_call(subdev, pad, get_fmt, NULL, &fmt);
        dev_info(dma->xdev->dev, "%s v4l2_subdev_call() done(ret=%d)\n", __func__, ret);
	if (ret < 0) {
                ret = (ret == -ENOIOCTLCMD) ? -EINVAL : ret;
                dev_info(dma->xdev->dev, "%s done(%d)\n", __func__, ret);
		return ret;
        }
	...
	if (dma->r.width != fmt.format.width ||
	    dma->r.height != fmt.format.height) {
                ret = -EINVAL;
                dev_info(dma->xdev->dev, "%s dma->r.width=%d\n", __func__, dma->r.width);
                dev_info(dma->xdev->dev, "%s fmt.format.width=%d\n", __func__, fmt.format.width);
                dev_info(dma->xdev->dev, "%s dma->r.height=%d\n", __func__, dma->r.height);
                dev_info(dma->xdev->dev, "%s fmt.format.width=%d\n", __func__, fmt.format.height);
                dev_info(dma->xdev->dev, "%s done(%d)\n", __func__, ret);
		return ret;
        }
        dev_info(dma->xdev->dev, "%s done(0)\n", __func__);
	return 0;
}
...
static void xvip_dma_buffer_queue(struct vb2_buffer *vb)
{
	...
	if (V4L2_TYPE_IS_MULTIPLANAR(dma->format.type)) {
		...
		xdbgvip_width_padding_factor(pix_mp->pixelformat,
					  &padding_factor_nume,
					  &padding_factor_deno);
		xdbgvip_bpl_scaling_factor(pix_mp->pixelformat, &bpl_nume,
					&bpl_deno);
		...
		xdbgvip_width_padding_factor(pix->pixelformat,
					  &padding_factor_nume,
					  &padding_factor_deno);
		xdbgvip_bpl_scaling_factor(pix->pixelformat, &bpl_nume,
					&bpl_deno);
		dma->xt.frame_size = dma->fmtinfo->num_planes;
		size = ((size_t)dma->r.width * dma->fmtinfo->bpl_factor *
			padding_factor_nume * bpl_nume) /
...
static int xvip_dma_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	pipe = to_xvip_pipeline(&dma->video) ? : &dma->pipe;
        ...
        dev_info(dma->xdev->dev, "%s video_device_pipeline_start() start\n", __func__);
	ret = video_device_pipeline_start(&dma->video, &pipe->pipe);
        dev_info(dma->xdev->dev, "%s video_device_pipeline_start() done(ret=%d)\n", __func__, ret);
	 ...
	 * connected subdev.
	 */
        dev_info(dma->xdev->dev, "%s xvip_dma_verify_format() start\n", __func__);
	ret = xvip_dma_verify_format(dma);
        dev_info(dma->xdev->dev, "%s xvip_dma_verify_format() done(ret=%d)\n", __func__, ret);
	if (ret < 0)
		goto error_stop;

        dev_info(dma->xdev->dev, "%s xvip_pipeline_prepare() start\n", __func__);
	ret = xvip_pipeline_prepare(pipe, dma);
        dev_info(dma->xdev->dev, "%s xvip_pipeline_prepare() done(ret=%d)\n", __func__, ret);
	if (ret < 0)
		goto error_stop;
	...
	if (!dma->low_latency_cap) {
		...
		/* Start the pipeline. */
                dev_info(dma->xdev->dev, "%s xvip_pipeline_set_stream() start\n", __func__);
		ret = xvip_pipeline_set_stream(pipe, true);
                dev_info(dma->xdev->dev, "%s xvip_pipeline_set_stream() done(ret=%d)\n", __func__, ret);
		if (ret < 0)
			goto error_stop;
	} else {
        ...
        dev_info(dma->xdev->dev, "%s done(ret=0)", __func__);
	return 0;
...
error:
	...
	spin_unlock_irq(&dma->queued_lock);
        ...
        dev_info(dma->xdev->dev, "%s done(ret=%d)", __func__, ret);
	return ret;
}
			      ...
			      struct v4l2_subdev_format *v4l_fmt)
{
	...
	if (v4l_fmt->format.code != dma->remote_subdev_med_bus ||
		...
		for (i = 0; i < fmt_cnt; i++) {
			fmt = xdbgvip_get_format_by_fourcc(fmts[i]);
			if (IS_ERR(fmt))
				return PTR_ERR(fmt);
	...
	fmt = xdbgvip_get_format_by_fourcc(dma->poss_v4l2_fmts[f->index]);
	if (IS_ERR(fmt))
		return PTR_ERR(fmt);
...
xvip_dma_enum_format(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	...
	fmt = xdbgvip_get_format_by_code(v4l_fmt.format.code);
	if (IS_ERR(fmt))
		return PTR_ERR(fmt);
		      ...
		      const struct xvip_video_format **fmtinfo)
{
	...
	info = xdbgvip_get_format_by_fourcc(fourcc);

	if (IS_ERR(info))
		info = xdbgvip_get_format_by_fourcc(XVIP_DMA_DEF_FORMAT);

	xdbgvip_width_padding_factor(info->fourcc, &padding_factor_nume,
				  &padding_factor_deno);
	xdbgvip_bpl_scaling_factor(info->fourcc, &bpl_nume, &bpl_deno);
		  ...
		  enum v4l2_buf_type type, unsigned int port)
{
	...
	dma->fmtinfo = xdbgvip_get_format_by_fourcc(XVIP_DMA_DEF_FORMAT);
