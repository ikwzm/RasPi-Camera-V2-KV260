...
static const struct of_device_id xdbg_scaler_of_id_table[] = {
	{ .compatible = "xlnx-dbg,v-vpss-scaler",
		.data = &xlnx_scaler},
	{ .compatible = "xlnx-dbg,v-vpss-scaler-1.0",
		.data = &xlnx_scaler_v1_0},
	{ .compatible = "xlnx-dbg,v-vpss-scaler-2.2",
		.data = &xlnx_scaler_v2_2},
	{ /* end of table */ }
};
MODULE_DEVICE_TABLE(of, xdbg_scaler_of_id_table);
	...
	for (i = 0; i < nphases; i++) {
		for (j = 0; j < ntaps / 2; j++) {
			...
			val = (xscaler->hscaler_coeff[i][rd_indx + 1] <<
			 ...
			 xdbgvip_write(&xscaler->xvip, base_addr +
				    ((i * ntaps / 2 + j) * 4), val);
		}
	}
}
...
{
	xdbgvip_clr(xvip, ((channel - 1) * XGPIO_CHAN_OFFSET) +
...
{
	xdbgvip_set(xvip, ((channel - 1) * XGPIO_CHAN_OFFSET) +
		 XGPIO_DATA_OFFSET + S_AXIS_RESET_OFF,
		 ip_block);
}
...
xv_vscaler_setup_video_fmt(struct xscaler_device *xscaler, u32 code_in)
{
	...
	xdbgvip_write(&xscaler->xvip, V_VSCALER_OFF +
				      ...
				      u32 code_out, u32 vsc_out)
{
	...
	xdbgvip_write(&xscaler->xvip, V_HSCALER_OFF +
...
xv_hscaler_set_phases(struct xscaler_device *xscaler)
{
	...
	case XSCALER_PPC_1:
		...
		for (i = 0; i < loop_width; i += 2) {
			...
			msb = lower_32_bits(xscaler->H_phases[i + 1] &
					    XHSC_MASK_LOW_16BITS);
			val = (msb << 16 | lsb);
			xdbgvip_write(&xscaler->xvip, offset + (index * 4), val);
			++index;
		}
		dev_dbg(xscaler->xvip.dev,
	...
	case XSCALER_PPC_2:
		...
		for (i = 0; i < loop_width; i++) {
			...
			xdbgvip_write(&xscaler->xvip, offset + (i * 4), val);
		}
		dev_dbg(xscaler->xvip.dev,
	...
	case XSCALER_PPC_4:
		...
		for (i = 0; i < loop_width; i++) {
			...
			xdbgvip_write(&xscaler->xvip, offset + (j * 4), lsb);
			xdbgvip_write(&xscaler->xvip, offset + ((j + 1) * 4), msb);
			j += 2;
		}
		dev_dbg(xscaler->xvip.dev,
			      ...
			      struct v4l2_subdev_format *fmt)
{
	...
	struct v4l2_mbus_framefmt *format;
        ...
        dev_info(xscaler->xvip.dev, "%s start\n", __func__);
	format = __xscaler_get_pad_format(xscaler, sd_state, fmt->pad,
					  fmt->which);
	if (!format) {
                dev_info(xscaler->xvip.dev, "%s done(-EINVAL)\n", __func__);
		return -EINVAL;
        }

	fmt->format = *format;

        dev_info(xscaler->xvip.dev, "%s done(0)\n", __func__);
	return 0;
}
...
static int xscaler_probe(struct platform_device *pdev)
{
	...
	match = of_match_node(xdbg_scaler_of_id_table, node);
	if (!match)
		return -ENODEV;

	if (!strcmp(xdbg_scaler_of_id_table[0].compatible, match->compatible)) {
		dev_warn(&pdev->dev,
	...
	if (xscaler->cfg->flags & XSCALER_CLK_PROP) {
		...
		ret = xdbgvip_init_resources(&xscaler->xvip);
		if (ret < 0)
			return ret;
	}
...
res_cleanup:
	xdbgvip_cleanup_resources(&xscaler->xvip);
	return ret;
}
...
static struct platform_driver xdbg_scaler_driver = {
	.driver			= {
		.name		= "xilinx-debug-vpss-scaler",
		.of_match_table	= xdbg_scaler_of_id_table,
	},
	.probe			= xscaler_probe,
	.remove			= xscaler_remove,
};

module_platform_driver(xdbg_scaler_driver);
