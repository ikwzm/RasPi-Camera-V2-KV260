...
#include "xilinx-debug-gamma-coeff.h"
...
static inline void xg_write(struct xgamma_dev *xg, u32 reg, u32 data)
{
	...
	xdbgvip_write(&xg->xvip, reg, data);
#ifdef DEBUG
			 ...
			 struct v4l2_subdev_format *fmt)
{
	...
	struct v4l2_mbus_framefmt *format;
        ...
        dev_info(xg->xvip.dev, "%s start\n", __func__);

	format = __xg_get_pad_format(xg, sd_state, fmt->pad, fmt->which);
	if (!format) {
                dev_info(xg->xvip.dev, "%s done(-EINVAL)\n", __func__);
		return -EINVAL;
        }

	fmt->format = *format;

        dev_info(xg->xvip.dev, "%s done(0)\n", __func__);
	return 0;
}
...
static int xg_probe(struct platform_device *pdev)
{
	...
	if (rval < 0)
		return rval;
	rval = xdbgvip_init_resources(&xg->xvip);
	if (rval)
		return -EIO;
...
media_error:
	xdbgvip_cleanup_resources(&xg->xvip);
	return rval;
}
...
static const struct of_device_id xg_of_id_table[] = {
	{.compatible = "xlnx-dbg,v-gamma-lut"},
	{ }
};
MODULE_DEVICE_TABLE(of, xg_of_id_table);

static struct platform_driver xdbg_g_driver = {
	.driver = {
		.name = "xilinx-debug-gamma-lut",
...
module_platform_driver(xdbg_g_driver);
